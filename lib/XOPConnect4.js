...
const { checkForUpdates } = require('../functions/XOPFunctions');
...
module.exports = class XOPConnect4Game {
    ...
    async startGame() {
        ...
        const askMsg = await this.message.lineReplyNoMention({ embed: embed, components: [row] });
        ...
        interaction.on('collect', async btn => {
            ...
            if (btn.id === 'reject') {
                ...
                this.message.lineReplyNoMention({ embed: this.GameEmbed() }).then(msg => {
                    this.gameEmbed = msg;
