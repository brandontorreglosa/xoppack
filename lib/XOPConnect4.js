...
module.exports = class XOPConnect4Game {
    constructor(options = {}) {
        ...
        if (typeof options.askerMessage !== 'string') throw new TypeError('Invalid_Message: Asker Message Must Be A Message!')
        if (!options.cancelMessage) options.cancelMessage = 'Looks Like They Didn\`t Want To Play!';
        if (typeof options.cancelMessage !== 'string') throw new TypeError('Invalid_Message: Cancel Message Must Be A Message!')
        if (!options.timerEndMessage) options.timerEndMessage = 'Since The Opponent Didnt Answer, I Ended The Game!';
        if (typeof options.timerEndMessage !== 'string') throw new TypeError('Invalid_Message: Timer End Message Must Be A Message!')
        if (!options.turnMessage) options.turnMessage = 'Its Now **{player}** Turn!';
        if (typeof options.turnMessage !== 'string') throw new TypeError('Invalid_Message: Turn Message Must Be A Message!')
        if (!options.gameEndMessage) options.gameEndMessage = 'The Game Ended Since No One Reasponsed!';
        if (typeof options.gameEndMessage !== 'string') throw new TypeError('Invalid_message: Game End Message Must Be A Message!')
        if (!options.winMessage) options.winMessage = '**{winner}** Won The Game!';
        if (typeof options.winMessage !== 'string') throw new TypeError('Invalid_Message: Win Message Must Be A Message!')
        if (!options.drawMessage) options.drawMessage = 'The Game Ended With A Draw!';
        if (typeof options.drawMessage !== 'string') throw new TypeError('Invalid_Message: Draw Message Must Be A Message!')
    ...
    getGameBoard() {
        ...
        for (let y = 0; y < HEIGHT; y++) {
            for (let x = 0; x < WIDTH; x++) { str += "" + gameBoard[y * WIDTH + x]; }
            str += "\n";
        }
    ...
    async startGame() {
        ...
        const noboyplay = new MessageEmbed().setTimestamp().setColor(this.options.embed.color).setAuthor("Connect 4", this.message.author.displayAvatarURL({ dynamic: true })).setDescription(`**${emoji}Sorry But You Cant Play With Bots!**`).setFooter('XOPPACK©', this.message.author.displayAvatarURL({ dynamic: true }))
        ...
        interaction.on('collect', async btn => {
            ...
            if (btn.id === 'reject') {
                ...
                askMsg.delete().catch();
                for (let y = 0; y < HEIGHT; y++) { for (let x = 0; x < WIDTH; x++) { gameBoard[y * WIDTH + x] = "⚪"; } }
                this.inGame = true;
                this.message.channel.send({ embed: this.GameEmbed() }).then(msg => {
                    this.gameEmbed = msg;
                    Object.keys(reactions).forEach(reaction => { this.gameEmbed.react(reaction); });
                    this.checkReactions();
                });
            }
        })
    ...
    GameEmbed() {
        ...
        return new MessageEmbed().setTimestamp().setColor(this.options.embed.color).setAuthor("Connect 4", this.message.author.displayAvatarURL({ dynamic: true })).setDescription(this.getGameBoard()).addField('Status', status).setFooter('XOPPACK©', this.message.author.displayAvatarURL({ dynamic: true }))
    }
    gameOver(result) {
        this.inGame = false;
        const editEmbed = new MessageEmbed().setTimestamp().setColor(this.options.embed.color).setAuthor("Connect 4", this.message.author.displayAvatarURL({ dynamic: true })).setDescription(this.getGameBoard()).addField('Status', this.getResultText(result)).setFooter('XOPPACK©', this.message.author.displayAvatarURL({ dynamic: true }))
    ...
    checkReactions() {
        ...
        this.gameEmbed.awaitReactions((reaction, user) => filter(reaction, user), { max: 1, time: 120000, errors: ['time'] }).then(async collected => {
            ...
            reaction.users.remove(user.id).then(() => {
                ...
                if (this.hasWon(placedX, placedY)) { this.gameOver({ result: 'winner', name: user.username, emoji: this.getChip() }); } else if (this.isBoardFull()) { this.gameOver({ result: 'tie' }); } else {
                    this.redTurn = !this.redTurn;
                    this.gameEmbed.edit({ embed: this.GameEmbed() });
                    this.checkReactions();
                }
            });
        }).catch(collected => { this.gameOver({ result: 'timeout' }); });
    }
