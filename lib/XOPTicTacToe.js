...
class TicTacToe {
    ...
    async startGame() {
        ...
        if (opponent.bot) return this.message.lineReplyNoMention(`**${emoji} You can't play with bots!**`)
        if (opponent.id === author.id) return this.message.lineReplyNoMention(`**${emoji} You cannot play with yourself!**`)
    ...
    async TTTGame() {
        ...
        var msg = await this.message.lineReplyNoMention({
        ...
        collector.on('collect', async btn => {
            ...
            if (btn.clicker.user.id !== turn) {
                return btn.reply.send(this.options.waitMessage, true)
            }
            await btn.reply.defer();
