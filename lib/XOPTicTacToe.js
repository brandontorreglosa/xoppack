...
class TicTacToe {
    ...
    async startGame() {
        ...
        const noboyplay = new MessageEmbed().setTimestamp().setColor(this.options.embed.color).setAuthor("Tic Tac Toe", this.message.author.displayAvatarURL({ dynamic: true })).setDescription(`**${emoji}Sorry But You Cant Play With Bots!**`).setFooter('XOPPACK©', this.message.author.displayAvatarURL({ dynamic: true }))
        const noyouplay = new MessageEmbed().setTimestamp().setColor(this.options.embed.color).setAuthor("Tic Tac Toe", this.message.author.displayAvatarURL({ dynamic: true })).setDescription(`**${emoji}Are You Alright?You Can Not Play With Yourself!**`).setFooter('XOPPACK©', this.message.author.displayAvatarURL({ dynamic: true }))
        ...
        const embed = new MessageEmbed().setTimestamp().setAuthor("Tic Tac Toe", this.message.author.displayAvatarURL({ dynamic: true })).setDescription(this.options.askMessage.replace('{challenger}', this.message.author).replace('{opponent}', this.opponent)).setColor(this.options.embed.color).setFooter('XOPPACK©', this.message.author.displayAvatarURL({ dynamic: true }))
        ...
        interaction.on("end", async(c, r) => {
            ...
            for (let y = 0; y < askMsg.components[0].components.length; y++) { askMsg.components[0].components[y].disabled = true; }
            askMsg.embeds[0].description = this.options.timeEndMessage.replace('{opponent}', this.opponent).replace('{challenger}', this.message.author);
            return askMsg.edit({ embed: askMsg.embeds[0], components: askMsg.components });
        })
    }
    async TTTGame() {
        ...
        for (let y = 0; y < 3; y++) { for (let x = 0; x < 3; x++) { gameBoard[x][y] = NO_MOVE; } }
        let btn_a1 = 'a1_' + i(10)
        let btn_a2 = 'a2_' + i(10)
        let btn_a3 = 'a3_' + i(10)
        let btn_b1 = 'b1_' + i(10)
        ...
        collector.on('collect', async btn => {
            ...
            if (btn.clicker.user.id !== turn) { return btn.reply.send(this.options.waitMessage, true) }
            await btn.reply.defer();
            const index = reactions[btn.id.split('_')[0]] - 1;
            const x = index % 3;
            const y = Math.floor(index / 3);
            const ebtn = msg.components[y].components[x];
            var new_btn = new MessageButton().setID(ebtn.custom_id).setDisabled().setEmoji(this.getChip()).setStyle(this.xTurn ? this.options.xColor : this.options.oColor)
            msg.components[y].components[x] = new_btn;
            this.placeMove(x, y, this.xTurn ? PLAYER_1 : PLAYER_2)
            if (this.isGameOver()) {
                if (this.hasWon(PLAYER_2))
                    this.gameOver({ result: "winner", name: this.opponent.username, emoji: this.getChip() }, msg);
                ...
                const edit_embed = msg.embeds[0].setDescription(this.options.turnMessage.replace('{emoji}', this.getChip()).replace('{player}', this.xTurn ? this.message.author.username : this.opponent.username)).setFooter('XOPPACK©', this.message.author.displayAvatarURL({ dynamic: true }))
    ...
    gameOver(result, msg) {
        this.inGame = false;
        const Embed = new MessageEmbed().setColor(msg.embeds[0].color).setAuthor(`${this.message.author.username}vs${this.opponent.username}`).setDescription("**Game Over!** - " + this.getResultText(result)).setFooter('XOPPACK©', this.message.author.displayAvatarURL({ dynamic: true }))
    ...
    isGameOver() {
        if (this.hasWon(PLAYER_1) || this.hasWon(PLAYER_2))
            return true;
        if (this.AvailablePoints().length == 0) { return true; }
        return false;
    }
    hasWon(player) {
        var gameBoard = this.gameBoard;
