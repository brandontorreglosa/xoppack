...
const { checkForUpdates, getRandomString } = require('../functions/XOPFunctions');

module.exports = async (options) => {
    checkForUpdates();
    if (!options.message) {
        throw new Error('Invalid_Message: Please Provide A Message For XOPPACK!');
    }
    if (typeof options.message !== 'object') {
        throw new TypeError('Invalid_Message: Invalid Discord Message Was Provide!');
    }
    if (!options.embed) options.embed = {};
    if (typeof options.embed !== 'object') {
        throw new TypeError('Invalid_Object: Embed Must Be An Valid Object!');
    }
    if (!options.embed.title) {
        options.embed.title = 'Calculator';
    }
    if (typeof options.embed.title !== 'string') {
        throw new TypeError('Invalid_Message: Embed Title Must Be A Message!');
    }
    if (!options.embed.color) options.embed.color = '#34b7db';
    if (typeof options.embed.color !== 'string') {
        throw new TypeError('Invalid_Color: Embed Color Must Be Hex. c30202 Or RED .etc!');
    }
    if (!options.embed.timestamp) options.embed.timestamp = true;
    if (typeof options.embed.timestamp !== 'boolean') {
        throw new TypeError('Invalid_Boolean: Timestamp Must Be True/False!');
    }
    if (!options.disabledcalcQuery) { options.disabledcalcQuery = 'Calculator For XOPPACK Is Currently Disabled!'; }
    if (typeof options.disabledcalcQuery !== 'string') {
        throw new TypeError('Invalid_Message: disabledcalcQuery Must Be A Message!');
    }
    if (!options.invalidQuery) {
        options.invalidQuery = 'This Equation Is Not Valid!';
    }
    if (typeof options.invalidQuery !== 'string') {
        throw new TypeError('Invalid_Message: invalidQuery Must Be A Message!');
    }
    if (!options.othersuserMessage) {
        options.othersuserMessage = 'Only <@{{author}}> Can Use The Buttons!';
    }
    if (typeof options.othersuserMessage !== 'string') {
        throw new TypeError('Invalid_Message: othersuserMessage Must Be A Message!');
    }
    ...
    const embed = new Discord.MessageEmbed()
        ...
        .setFooter('XOPPACK©', message.author.displayAvatarURL({ dynamic: true }))
        ...
        calc.on('collect', async (btn) => {
            if (btn.clicker.user.id !== options.message.author.id) {
                return btn.reply.send(
                    options.othersuserMessage.replace(
                        '{{author}}',
