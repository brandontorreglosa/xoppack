...
class XOPRPSGame {
    constructor(options = {}) {
        ...
        if (typeof options.embed.title !== 'string') throw new TypeError('Invalid_Message: Embed Title Needs To Be A Message!')
    ...
    async startGame() {
        ...
        const noboyplay = new MessageEmbed().setTimestamp().setColor(this.options.embed.color).setAuthor(this.options.embed.title, this.message.author.displayAvatarURL({ dynamic: true })).setDescription(`**${emoji}Sorry But You Cant Play With Bots!**`).setFooter('XOPPACK©', this.message.author.displayAvatarURL({ dynamic: true }))
    ...
    async RPSGame() {
        ...
        let opponent_choice;
        const filter = m => m;
        const collector = msg.createButtonCollector(filter, { time: 120000, })
        collector.on('collect', async btn => {
            if (btn.clicker.user.id !== this.message.author.id && btn.clicker.user.id !== this.opponent.id) { return btn.reply.send(this.options.othersuserMessage, true) }
            if (btn.clicker.user.id == this.message.author.id) {
                if (challenger_choice) { return btn.reply.send(this.options.noChangeMessage, true) }
                ...
                if (opponent_choice) { return btn.reply.send(this.options.noChangeMessage, true) }
        ...
        collector.on("end", async(c, r) => {
            if (r === 'time' && this.inGame == true) {
                const endEmbed = new MessageEmbed().setAuthor(this.options.embed.title, this.message.author.displayAvatarURL({ dynamic: true })).setColor(this.options.embed.color).setDescription(this.options.gameEndMessage).setFooter('XOPPACK©', this.message.author.displayAvatarURL({ dynamic: true })).setTimestamp()
    ...
    getResult(msg, challenger, opponent) {
        let result;
        if (challenger === opponent) { result = this.options.drawMessage; } else if ((opponent === '✂️' && challenger === '📃') || (opponent === '🌑' && challenger === '✂️') || (opponent === '📃' && challenger === '🌑')) { result = this.options.winMessage.replace('{winner}', this.opponent) } else { result = this.options.winMessage.replace('{winner}', this.message.author) }
