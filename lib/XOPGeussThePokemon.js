...
module.exports = class XOPGuessThePokemon {
    constructor(options = {}) {
    ...
    async startGame() {
        ...
        const msg = await this.message.lineReplyNoMention({ embed: [embed] })
        ...
        collector.on('collect', (message) => {
            ...
            if (message.content.toLowerCase() === data.name.toLowerCase()) {
                ...
                return msg.channel.send({ content: this.options.winMessage.replace('{pokemon}', data.name), embed: [embed] })
            } else {
