...
const answers = ['Maybe.', 'Certainly not.', 'I hope so.', 'Not in your wildest dreams.', 'There is a good chance.', 'Quite likely.', 'I think so.', 'I hope not.', 'I hope so.', 'Never!', 'Fuhgeddaboudit.', 'Ahaha! Really?!?', 'Pfft.', 'Sorry, bucko.', 'Hell, yes.', 'Hell to the no.', 'The future is bleak.', 'The future is uncertain.', 'I would rather not say.', 'Who cares?', 'Possibly.', 'Never, ever, ever.', 'There is a small chance.', 'As I see it, yes', 'It is certain', 'It is decidedly so', 'Most likely', 'Outlook good', 'Signs point to yes', 'Yes!'];
module.exports = async(options) => {
    ...
    const embed = new MessageEmbed().setAuthor(options.embed.title, options.message.author.displayAvatarURL({ dynamic: true })).setColor(options.embed.color).addField('Question', options.question, false).addField('Answer', answers[Math.floor(Math.random() * answers.length)], false).setFooter('XOPPACK©', options.message.author.displayAvatarURL({ dynamic: true }))
